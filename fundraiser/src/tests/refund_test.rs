...
fn refund_test() {
    ...
    // Create the contributor account with the correct data length
    let contributor_account = crate::tests::create_account(
        mollusk.sysvars.rent.minimum_balance(crate::state::Contributor::LEN),
        crate::state::Contributor::LEN,
        &program_id,
    );
    ...
    let contribute_accounts = vec![
        ...
        (contributor_pda, contributor_account.clone()),
    ...
    let updated_contributor_account = contribute_result
    ...
    let refund_instruction = Instruction::new_with_bytes(
        ...
        vec![
            ...
            AccountMeta::new(contributor_pda, true),
            AccountMeta::new(signer_ta, true),
    ...
    let refund_accounts = vec![
        (fundraiser_pda, updated_fundraiser_account.clone()),
        (contributor_pda, updated_contributor_account.clone()),
        (signer_ta, signer_ta_account.clone()),
        (vault, updated_vault_account.clone()),
        (token_program, token_program_account.clone()),
    ];
    ...
    assert!(!refund_result.program_result.is_err(), "Refund failed");

    let updated_vault_account = refund_result.get_account(&vault).unwrap().clone();
    let updated_fundraiser_account = refund_result.get_account(&fundraiser_pda).unwrap().clone();
    assert_eq!(
